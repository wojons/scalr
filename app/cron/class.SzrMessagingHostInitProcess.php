...
{
    public $ThreadArgs;
    public $ProcessDescription = "SZR Messaging HostInit";
    public $Logger;
    ...
    public function OnStartForking()
    {
        ...
        if ($pid) {
            ...
            // Execute command
            $ps = $Shell->queryRaw("ps ax -o pid,ppid,command | grep ' 1' | grep {$pid} | grep -v 'ps x' | egrep 'SzrMessagingHostInit\$'");
    ...
    public function StartThread($data)
    {
        ...
        $job = new Scalr_Cronjob_ScalarizrMessagingHostInit();
        ...
        while(true) {
            $rows = $db->Execute("
                ...
                AND m.message_name = ?
            ", array(
                "in", MESSAGE_STATUS::PENDING, EVENT_TYPE::HOST_INIT
            ));
