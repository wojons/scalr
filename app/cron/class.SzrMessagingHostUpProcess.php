...
require_once dirname(__FILE__).'/../cron-ng/jobs/ScalarizrMessagingHostUp.php';
 ...
 */
class SzrMessagingHostUpProcess implements \Scalr\System\Pcntl\ProcessInterface
{
    public $ThreadArgs;
    public $ProcessDescription = "SZR Messaging HostUp";
    public $Logger;
    ...
    public function OnStartForking()
    {
        ...
        if ($pid) {
            ...
            // Execute command
            $ps = $Shell->queryRaw("ps ax -o pid,ppid,command | grep ' 1' | grep {$pid} | grep -v 'ps x' | egrep 'SzrMessagingHostUp\$'");
    ...
    public function StartThread($data)
    {
        ...
        $job = new Scalr_Cronjob_ScalarizrMessagingHostUp();
        ...
        while(true) {
            $rows = $db->Execute("
                ...
                AND m.message_name = ?
            ", array(
                "in", MESSAGE_STATUS::PENDING, EVENT_TYPE::HOST_UP
            ));
