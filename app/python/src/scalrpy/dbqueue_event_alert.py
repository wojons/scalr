...
CONFIG = {
    ...
    'interval':60,
    'log_file':'/var/log/scalr.dbqueue-event-alert.log',
    'pid_file':'/var/run/scalr.dbqueue-event-alert.pid',
    'verbosity':1,
    }
...
class DBQueueEventAlert(basedaemon.BaseDaemon):

    def __init__(self):
        super(DBQueueEventAlert, self).__init__(pid_file=CONFIG['pid_file'])
    ...
    def start(self, daemon=False):
        if daemon:
            super(DBQueueEventAlert, self).start()
        else:
            self.run()
