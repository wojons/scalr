...
class Analytics(object):
    ...
    def load_creds(self, envs, platforms=None):
        ...
        if platforms is None:
            platforms = Credential.scheme.keys()
        for platform in platforms:
            names = ['%s.%s' % (platform, element) for element in Credential.scheme[platform]]
            names += ['%s.is_enabled' % platform]
            if platform == 'ec2':
                names += ['ec2.account_id']
            ...
            for env_id, data in tmp.iteritems():
                if data['is_enabled'] == '0':
                    continue
                out.append(Credential(env_id, platform, data))
        return out
    ...
    def get_managed_servers(self, date, hour, limit=1000, force=False):
        """
        :returns: generator
        """
        ...
        for servers in self.analytics_db.execute_with_limit(query, limit, retries=1):
            ...
            self._load_managed_servers_properties(servers)
    ...
    def get_not_managed_servers(self, date, hour, limit=1000, force=False):
        """
        :returns: generator
        """
    ...
    def recalculate_usage_d(self, date, platform):
        ...
        query = (
                ...
                "SELECT date, platform, cc_id, project_id, farm_id, u.env_id, cost "
                "FROM (SELECT DATE(u.dtime) date,u.platform,"
                "u.cc_id, u.project_id, u.farm_id, SUM(u.cost) cost "
                "FROM usage_h u "
