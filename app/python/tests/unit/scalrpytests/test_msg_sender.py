...
class MsgSenderTest(unittest.TestCase):
    ...
    def test_db_update_ok(self):
        daemon = msg_sender.MsgSender()
        daemon._db.execute_query = mock.MagicMock()
        message = dict(
                messageid='id',
                server_id='id',
                event_id='id',
        ...
        assert daemon._db.execute_query.mock_calls[0][1][0] == \
                "DELETE FROM `messages` WHERE `messageid` IN ( 'id_exec' )"
        assert daemon._db.execute_query.mock_calls[1][1][0] == \
                "UPDATE `messages` SET `status`=1, `message`='', `dtlasthandleattempt`=NOW() WHERE `messageid` IN ( 'id', 'id' )" 
        assert daemon._db.execute_query.mock_calls[2][1][0] == \
