...
namespace Scalr\Modules\Platforms\Eucalyptus;
    ...
    public function CreateServerSnapshot(BundleTask $BundleTask)
    {
        ...
        if ($rootDeviceType == 'ebs') {
            ...
            if ($platfrom == 'windows') {
                ...
                if (!$DBServer->SendMessage($msg, false, true)) {
    ...
    public function LaunchServer(DBServer $DBServer, \Scalr_Server_LaunchOptions $launchOptions = null)
    {
        ...
        if ($result->instancesSet->get(0)->instanceId) {
            $DBServer->SetProperties([
                \EUCA_SERVER_PROPERTIES::AVAIL_ZONE    => $result->instancesSet->get(0)->placement->availabilityZone,
                \EUCA_SERVER_PROPERTIES::INSTANCE_ID   => $result->instancesSet->get(0)->instanceId,
                \EUCA_SERVER_PROPERTIES::INSTANCE_TYPE => $runInstanceRequest->instanceType,
                \EUCA_SERVER_PROPERTIES::EMIID         => $runInstanceRequest->imageId,
                \EUCA_SERVER_PROPERTIES::REGION        => $launchOptions->cloudLocation,
