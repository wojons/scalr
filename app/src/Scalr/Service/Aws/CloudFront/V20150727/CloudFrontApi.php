<?php
namespace Scalr\Service\Aws\CloudFront\V20150727;
...
use DateTime;
use DateTimeZone;
use Scalr\Service\Aws\CloudFront\DataType\CustomErrorResponseData;
use Scalr\Service\Aws\CloudFront\DataType\CustomErrorResponseList;
use Scalr\Service\Aws\CloudFront\DataType\GeoRestrictionData;
use Scalr\Service\Aws\CloudFront\DataType\ViewerCertificateData;
...
class CloudFrontApi extends AbstractApi
{
    ...
    /**
     * Loads CustomErrorResponseData from simple xml object
     *
     * @param   \SimpleXMLElement $sxml
     * @return  CustomErrorResponseData Returns CustomErrorResponseData
     */
    protected function _loadCustomErrorResponseData(\SimpleXMLElement $sxml)
    {
        $item = null;
        if ($this->exist($sxml)) {
            $item = new CustomErrorResponseData();
            $item->setCloudFront($this->cloudFront);
            $item->errorCachingMinTtl = $this->exist($sxml->ErrorCachingMinTTL) ? (int) $sxml->ErrorCachingMinTTL : null;
            $item->errorCode = $this->exist($sxml->ErrorCode) ? (string) $sxml->ErrorCode : null;
            $item->responseCode = $this->exist($sxml->ResponseCode) ? (string) $sxml->ResponseCode : null;
            $item->responsePagePath = $this->exist($sxml->ResponsePagePath) ? (string) $sxml->ResponsePagePath : null;
        }
        return $item;
    }
     ...
     *
     * @param   \SimpleXMLElement $sxml
     * @return  ViewerCertificateData Returns ViewerCertificateData
     */
     ...
     *
     * @param   \SimpleXMLElement $sxml
     * @return  GeoRestrictionData Returns GeoRestrictionData
     */
    ...
    /**
     * Parses CustomErrorResponseList
     *
     * @param   \SimpleXMLElement $sxml
     * @return  CustomErrorResponseList Returns CustomErrorResponseList object
     */
    protected function _loadCustomErrorResponseList(\SimpleXMLElement $sxml)
    {
        $list = new CustomErrorResponseList();
        $list->setCloudFront($this->cloudFront);
        if (!empty($sxml->Items->CustomErrorResponse)) {
            foreach ($sxml->Items->CustomErrorResponse as $v) {
                $list->append($this->_loadCustomErrorResponseData($v));
            }
        }
        return $list;
    }
