...
namespace Scalr\Service\Aws\Ec2\V20150415;
...
class Ec2Api extends AbstractApi
{
     ...
     *
     * @param   ListDataType               $zoneName optional Zone Name List to filter.
        ...
        if ($zoneName !== null) {
            if ($isSoap) {
                foreach ($zoneName as $v) {
                    $options['availabilityZoneSet']['item'][] = [
                        'zoneName' => $v
                    ];
                }
            } else {
     ...
     * @param   OfferingType               $offeringType           optional The Reserved Instance offering type.
     *                                                             If you are using tools that predate the 2011-11-01 API version, you only have access to the Medium Utilization Reserved Instance offering type.
    ...
    public function describeReservedInstances(ListDataType $reservedInstanceIdList = null,
                                              ReservedInstanceFilterList $filter = null, OfferingType $offeringType = null)
    {
        $result = null;
        $options = [];
        ...
        if ($offeringType !== null) {
            $options['OfferingType'] = (string) $offeringType;
        }
     ...
     * @throws  Ec2Exception
     */
    public function describeVolumes(ListDataType $volumeIdList = null, VolumeFilterList $filter = null, $nextToken = null, $maxResults = null)
    {
        $result = null;
        $options = [];
    ...
    protected function _loadVolumeData(\SimpleXMLElement $v)
    {
        ...
        if ($this->exist($v)) {
            ...
            $item->kmsKeyId = $this->exist($v->kmsKeyId) ? (string) $v->kmsKeyId : null;
            $item
                ->setTagSet($this->_loadResourceTagSetList($v->tagSet))
    ...
    public function describeSnapshots(ListDataType $snapshotIdList = null, ListDataType $ownerList = null,
                                      SnapshotFilterList $filter = null, ListDataType $restorableByList = null, $nextToken = null, $maxResults = null)
    {
        $result = null;
        $options = [];
    ...
    protected function _loadRouteData(\SimpleXMLElement $v)
    {
        ...
        if ($this->exist($v)) {
            ...
            $item->instanceOwnerId = $this->exist($v->instanceOwnerId) ? (string) $v->instanceOwnerId : null;
            $item->networkInterfaceId = $this->exist($v->networkInterfaceId) ? (string) $v->networkInterfaceId : null;
            $item->vpcPeeringConnectionId = $this->exist($v->vpcPeeringConnectionId) ? (string) $v->vpcPeeringConnectionId : null;
        }
        return $item;
    }
     ...
     * @param   string     $vpcPeeringConnectionId optional The ID of a VPC peering connection.
     * @param   string     $clientToken          optional Unique, case-sensitive identifier you provide to ensure the idempotency of the request.
     * @return  bool       Returns TRUE on success
     * @throws  ClientException
     * @throws  Ec2Exception
     */
    public function createRoute($routeTableId, $destinationCidrBlock, $gatewayId = null, $instanceId = null,
                                $networkInterfaceId = null, $vpcPeeringConnectionId = null, $clientToken = null)
    {
        $result = false;
        ...
        if ($f > 1) {
            throw new \InvalidArgumentException(sprintf(
                'You must provide only one of the following: a GatewayId, InstanceId, VpcPeeringConnectionId or NetworkInterfaceId.'
            ));
        }
