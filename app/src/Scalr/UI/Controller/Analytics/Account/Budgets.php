...
class Scalr_UI_Controller_Analytics_Account_Budgets extends Scalr_UI_Controller
{
    ...
    {
        $this->response->page('ui/analytics/account/budgets/view.js', array(
            'quarters' => SettingEntity::getQuarters(true),
            'quartersConfirmed' => SettingEntity::getValue(SettingEntity::ID_QUARTERS_DAYS_CONFIRMED)
        ), array('ui/analytics/account/account.js'), array('ui/analytics/analytics.css', '/ui/analytics/admin/admin.css', '/ui/analytics/account/budgets/budgets.css'));
    }
    ...
    protected function getBudgetInfo($year, $ccId = null, $projectId = null)
    {
        ...
        if ($subjectEntity->accountId !== $this->environment->clientId) {
    ...
    private function getNodesList($period, $ccId = null, $query = null)
    {
        ...
        $projectItems = $this->getContainer()->analytics->projects->findByKey($query, ['accountId' => $this->environment->clientId], true);
        ...
        $ccsItems = $this->getContainer()->analytics->ccs->findByKey($query, ['accountId' => $this->environment->clientId], true);
            ...
            if (!isset($nodes[$item->ccId])) {
                ...
                foreach ($projectItems as $projectItem) {
                    ...
                    if ($projectItem->shared != ProjectEntity::SHARED_WITHIN_ACCOUNT ||
                        $projectItem->accountId != $this->environment->clientId) {
                        continue;
                    }
    ...
    public function xSaveAction()
    {
        ...
        if ($this->getParam('projectId')) {
            ...
            /* @var ProjectEntity $subjectEntity */
            if ($subjectEntity->accountId != $this->environment->clientId || $subjectEntity->shared != ProjectEntity::SHARED_WITHIN_ACCOUNT) {
                throw new Scalr_Exception_InsufficientPermissions();
            }
        } else {
