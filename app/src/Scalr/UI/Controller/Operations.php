    ...
    public function detailsAction($serverId, $operationId = null, $operation = null)
    {
        ...
        if ($operationId) {
            ...
            if ($isInitFailed) {
                ...
                $details[$this->getScalarizrPhaseName($errorEvent)]['status'] = 'error';
                $details[$this->getScalarizrPhaseName($errorEvent)]['message'] = htmlspecialchars($dbServer->GetProperty(SERVER_PROPERTIES::SZR_IS_INIT_ERROR_MSG));
                $download = true;
                $opStatus = 'Failed';
            }
