...
if [ "$rhel" -eq 0 ] && [ "$fedora" -eq 0 ] && [ "$amazon" -eq 0 ]; then
    ...
    action "Installing essential packages" "rm -r /var/lib/apt/lists/* ; apt-get update ; apt-get install -y curl git libc6"
else
...
action "Installing chef" 'chmod +x install.sh; /tmp/install.sh -v 12.1'
