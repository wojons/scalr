    ...
    if ($req_FarmID && $req_Hash)
    {
        ...
        if ($DBServer->localIp == $_SERVER['REMOTE_ADDR'] && $DBServer->platform == SERVER_PLATFORMS::EC2)
        {
            if (!$DBServer->remoteIp)
            {
                try
                {
                    $ips = \Scalr\Modules\PlatformFactory::NewPlatform($DBServer->platform)->GetServerIPAddresses($DBServer);
        ...
        if ($DBFarm && $DBServer)
        {
            ...
            switch ($req_EventType)
            {
                ...
                case "logEvent":
                    ...
                    $db->Execute("INSERT DELAYED INTO logentries SET id=?, serverid=?, message=?, `time`=?, severity=?, `source`=?, farmid=? ON DUPLICATE KEY UPDATE cnt = cnt + 1, `time` = ?",
                        array($hash, $DBServer->serverId, $message, time(), $data["severity"], $data["source"], $req_FarmID, time())
                    );
