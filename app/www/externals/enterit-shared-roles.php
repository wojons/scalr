    ...
    try {
		foreach ($roles as $role) {
            ...
            */
		    ...
		    $found = false;
		    foreach ($role['role_images'] as $img) {
		        if ($img['platform'] == 'ecs')
		            $found = true;
		    }
			...
			$chk = $db->GetOne("SELECT id FROM roles WHERE origin=? AND name=? LIMIT 1", array($role['origin'], $role['name']));
			if (!$chk) {

			    print "Adding new role: '{$role['name']}'\n";
