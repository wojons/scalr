Scalr.regPage('Scalr.ui.account2.billing.details', function (loadParams, moduleParams) {
    ...
    var getNextCharge = function()
    {
        if (moduleParams['billing']['nextAssessmentAt']) {
            if (moduleParams['billing']['ccType'])
                return '$'+moduleParams['billing']['nextAmount']+' on '+moduleParams['billing']['nextAssessmentAt']+' on '+moduleParams['billing']['ccType']+' '+moduleParams['billing']['ccNumber']+' [<a href="#/account/billing/updateCreditCard">Change card</a>]'
            else
                return '<a href="#/account/billing/updateCreditCard">Enter credit card info to get a paid account</a>'
        } else {
            return "";
        }
    }
    ...
    var getState = function()
    {
        if (moduleParams['billing']['state'] == 'Subscribed')
            return '<span class="x-semibold" style="color:green;">Subscribed</span>';
        else if (moduleParams['billing']['state'] == 'Trial')
            return '<span class="x-semibold" style="color:green;">Trial</span> (<b>' + moduleParams['billing']['trialDaysLeft'] + '</b> days left)';
        else if (moduleParams['billing']['state'] == 'Unsubscribed' && moduleParams['billing']['ccType'])
            return '<span class="x-semibold" style="color:red;">Unsubscribed</span> [<a href="#/account/billing/reactivate">Re-activate</a>]';
        else if (moduleParams['billing']['state'] == 'Unsubscribed' && !moduleParams['billing']['ccType'])
            return '<span class="x-semibold" style="color:red;">Unsubscribed</span> [<a href="#/account/billing/updateCreditCard">Set credit card & Re-activate</a>]';
        else if (moduleParams['billing']['state'] == 'Behind on payment')
            return '<span class="x-semibold" style="color:red;">Behind on payment</span>'
    }
    ...
    var getPlanDetails = function()
    {
        if (moduleParams['billing']['state'] == 'Unsubscribed' && !moduleParams['billing']['ccType'])
            ...
            if (!moduleParams['billing']['scu'])
                var plan = moduleParams['billing']['productName'] + " ( $"+moduleParams['billing']['productPrice']+" / month) [<a href='#/account/billing/changePlan'>Change plan / Cancel subscription</a>]"
            else
                var plan = moduleParams['billing']['productName'] + " ( $"+moduleParams['billing']['productPrice']+" / month) [<a href='#/account/billing/changePlan'>Cancel subscription</a>]"
        }

        return plan;
    };

    if (! ('scu' in moduleParams['billing']))
        moduleParams['billing']['scu'] = {};

    var couponString = (moduleParams['billing']['couponCode']) ? moduleParams['billing']['couponDiscount'] + ' (Used coupon: ' + moduleParams['billing']['couponCode']+')' : "No discount [<a href='#/account/billing/applyCouponCode'>enter coupon code</a>]";

    var panel = Ext.create('Ext.form.Panel', {
        scalrOptions: {
            maximize: 'all',
            menuTitle: 'Billing',
            menuHref: '#/account/billing',
            menuFavorite: true
        },
        stateId: 'panel-account-billing',
        stateful: true,
        fieldDefaults: {
            anchor: '100%',
            labelWidth: 140,
        ...
        items: [{
            ...
            items: [{
                ...
                value: "You're under an old plan that doesn't allow for metered billing. If you want to get access to the new features we recently announced, <a href='#/account/billing/changePlan'>please upgrade your subscription</a>.",
            }, {
                ...
                "<br/><a href='#/account/billing/changePlan'>Click here to switch to direct CC billing</a>, and have your subscription to paypal canceled.",
            }, {
                xtype: 'displayfield',
                fieldLabel: 'Plan',
            ...
            hidden: !(moduleParams['billing']['ccType']),
            defaults: {
                labelWidth: 140
            },
            items: [{
            ...
            items: [{
                ...
                xtype: 'displayfield',
                cls: 'x-form-field-warning',
                value: "You're using more servers than allowed by your plan. <a href='#/account/billing/changePlan'>Click here to upgrade your subscription</a>."
            }, {
                xtype: 'fieldcontainer',
                fieldLabel: 'Servers',
                layout: 'hbox',
                items: [{
                    ...
                    margin: '0 0 0 3',
                    value: '[<a href="#/account/billing/changePlan">Increase limit</a>]'
                }]
            }, {
                xtype: 'fieldcontainer',
                fieldLabel: 'Farms',
                ...
                items: [{
                 ...
                 xtype: 'displayfield',
                 margin: '0 0 0 3',
                 value: '[<a href="#/account/billing/buyEnvironments">Buy more for $99 / environment / month</a>]'
                 }*/]
            }, {
                xtype: 'fieldcontainer',
            ...
            items: [{
                ...
                handler: function() {
                    Scalr.event.fireEvent('redirect', '#/account/billing/invoicesList');
                }
            }]
        }]
    });
