Scalr.regPage('Scalr.ui.analytics.admin.dashboard.view', function (loadPararams, moduleParams) {
	...
	var panel = Ext.create('Ext.panel.Panel', {
		...
		items: [{
            ...
            items: [{
                ...
                hidden: true,
                html: '<img src="' + Ext.BLANK_IMAGE_URL + '" class="x-icon-warning"/>&nbsp;&nbsp;<a class="x-link-warning" href="#">Scalr has detected some unallocated costs!</a>',
                listeners: {
                    render: function() {
                        var me = this;
    ...
    showChart: function(data, enabledSeries) {
        ...
        series.push({
            ...
            renderer: function(sprite, record, attr, index, store){
                ...
                Ext.apply(attr, {fill: Scalr.utils.getColorById(name, 'clouds')});
                return attr;
            },
            highlight: {
                stroke: null,
            ...
            tips: {
                ...
                renderer: function(record, item) {
                    ...
                    if (info) {
                        this.update({
                            name: Scalr.utils.getPlatformName(item.yField),
        ...
        chartCt.add({
            ...
            toggleSeries: function(name, enabled) {
                Ext.Array[enabled ? 'include' : 'remove'](this.series.getAt(0).yField, name);
                this.refresh();
    ...
    items: [{
        ...
        items: [{
            ...
            items: [{
                xtype: 'component',
                itemId: 'title',
                html: '<div class="x-cabox-title"><div class="separator-right" style="float:left;width:200px;">Total spend</div><div style="float:left;margin-left:180px;">By cloud</div></div>'
            },{
                xtype: 'container',
                layout: {
                    type: 'hbox',
                ...
                items: [{
                     ...
                     width: 200,
                     tpl:
                        '<div class="title1" style="margin:16px 0" <tpl if="unassignedCost">data-qtip="Includes {[this.currency(values.unassignedCost)]} unaccounted by any cost center"</tpl>>{[this.currency(values.cost)]}</div>' +
                        '<div style="margin:0 0 6px">Last period ({prevPeriod})</div>' +//Same time previous {period}:
                        '<div class="title2">{[this.currency(values.prevCost)]}</div>' +
                        '<tpl if="growth!=0">' +
                            '<div style="margin:20px 0 6px">Growth</div>' +
                    ...
                    series: [{
                        ...
                        donut: 24,
                        renderer: function(sprite, record, attr, index, store){
                            return Ext.apply(attr, {fill: Scalr.utils.getColorById(record.get('id'), 'clouds')});
                        },
                        tips: {
                            trackMouse: true,
                            hideDelay: 0,
                    ...
                    tpl: new Ext.XTemplate(
                        '<tpl for=".">' +
                            '<div class="x-item">' +
                                ...
                                '<div style="margin:0 0 0 30px;"><span style="font-weight:bold;position:relative;top:2px;" <tpl if="unassignedCost">data-qtip="Includes {[this.currency(values.unassignedCost)]} unaccounted by any cost center"</tpl>>{[this.currency(values.cost)]}</span>&nbsp; ' +
                                '<tpl if="growth!=0">' +
                        ...
                        '<tpl for=".">' +
                            '<tpl if="xindex&lt;6">' +
                                '<tr class="x-top5-item">' +
                                    '<td>' + 
                                        '<tpl if="id">' +
                                            '<a href="#/analytics/{[this.type]}?{[this.type==\'costcenters\'?\'cc\':\'project\']}Id={id}">' +
                                        '</tpl>' +
