Scalr.regPage('Scalr.ui.analytics.budgets.view', function (loadParams, moduleParams) {
	...
	var store = Ext.create('Ext.data.TreeStore', {
        ...
        listeners: {
            ...
            load: function() {
                var data = this.proxy.reader.jsonData;
                if (data['success']) {
                    ...
                    treepanel.down('#header').setVisible(!Ext.isEmpty(this.getRootNode().childNodes));
                    treepanel.down('#headerDates').update((data['quarter'] === 'year' ? 'Q1 - Q4' : ('Q' + data['quarter'])) + dates);
                    treepanel.down('#headerCurDate').update(this.proxy.extraParams['quarter'] !== 'year' && Ext.Date.between(curDate, startDate, endDate) ? {
                        date: Ext.Date.format(curDate, 'M j'),
                        offset: getDateOffset(startDate, endDate, curDate)
                    } : '');
                    treepanel.el.unmask();
                } else {
                    Scalr.message.Error(this.proxy.reader.jsonData['errorMessage']);
                }
            }
        },
		proxy: {
			type: 'ajax',
			url: '/analytics/budgets/xList',
            reader: {
                type: 'json',
                root: 'nodes'
            }
    ...
    var treepanel = Ext.widget({
        ...
        dockedItems: [{
            ...
            items: [{
                ...
                href: '#/analytics/budgets/quarterCalendar',
                hrefTarget: '_self'
            }]
        },{
            xtype: 'container',
    ...
    var form = Ext.create('Ext.form.Panel', {
        ...
        loadBudgetInfo: function(record) {
            ...
            me.currentRequest = Scalr.Request({
                url: '/analytics/budgets/xGetBudgetInfo',
                params: {
        ...
        showBudgetInfo: function(data) {
                    ...
                    items: [{
                        ...
                        items: [{
                            items: [{
                                ...
                                cls: 'x-caheader',
                                margin: '10 0 0',
                                html: Ext.util.Format.currency(budget['budget'], null, 0)
                            } : {
                                xtype: 'textfield',
                                name: 'budget' + (index + 1),
                                fieldLabel: '$',
                            ...
                            items: [{
                                ...
                                items: [{
                                    ...
                                    items: [{
                                        ...
                                        html: Ext.util.Format.currency(budget['prev']['budget'], null, 0)
                                    }]
                                },{
                                    xtype: 'container',
                                    flex: 1,
            ...
            this.getDockedComponent('buttons').setVisible(!allQuartersClosed);
            this.up().unmask();
            this.show();
        },
        items: [{
            ...
            items: [{
                ...
                handler: function() {
                    ...
                    Scalr.Request({
                        ...
                        url: '/analytics/budgets/xSave/',
                        success: function (data) {
                            Scalr.event.fireEvent('update', '/analytics/budgets/edit');
