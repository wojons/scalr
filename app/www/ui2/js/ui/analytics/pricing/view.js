Scalr.regPage('Scalr.ui.analytics.pricing.view', function (loadParams, moduleParams) {

    var endpointsStore = Ext.create('store.store', {
        fields: ['envId', 'url'],
        proxy: {
            type: 'ajax',
            url: '/analytics/pricing/xGetPlatformEndpoints',
            reader: {
                type: 'json',
                root: 'data'
            }
    ...
    var locationsStore = Ext.create('store.store', {
        ...
        proxy: {
            type: 'ajax',
            url: '/analytics/pricing/xGetPlatformLocations',
            reader: {
                type: 'json',
                root: 'data'
            }
    ...
    var pricesStore = Ext.create('store.store', {
        ...
        proxy: {
            type: 'ajax',
            url: '/analytics/pricing/xGetPlatformInstanceTypes',
            reader: {
                type: 'json',
                root: 'data.prices'
            }
        ...
        changePrice: function (newPrice, instanceType, priceType) {
            var me = this;
    ...
    var pricingPanel = Ext.create('Ext.panel.Panel', {
        ...
        getUrl: function () {
            var me = this;
            return me.platform !== 'ec2' ? (me.down('[name=endpoints]').getRawValue() || '') : '';
        },

        getEnvId: function () {
            var me = this;
            return me.platform !== 'ec2' ? (me.down('[name=endpoints]').getValue() || 0) : 0;
        },
        ...
        loadEc2: function () {
            ...
            me.loadLocations(0, '').
                down('[name=endpoints]').setValue('Amazon EC2');
        ...
        savePrices: function (params, callback, withoutConfirm) {
            ...
            Scalr.Request({
                url: '/analytics/pricing/xSavePrice',
        ...
        loadHistory: function (callback) {
            ...
            Scalr.Request({
                url: '/analytics/pricing/xGetPlatformPricingHistory',
                processBox: {
                    type: 'action'
                },
                params: params,
        ...
        setPlatform: function (platform) {
            ...
            var isEc2 = me.platform === 'ec2';

            me.clearEndpoints().clearLocations().clearPrices().
                toggleAutomaticUpdate(isEc2);
        ...
        dockedItems: [
            {
                ...
                name: 'platforms',
                dock: 'left',
                cls: 'scalr-ui-analytics-pricing-platform-buttons',
                width: 180,
                overflowY: 'auto',
                overflowX: 'hidden',
            ...
            columns: [
                ...
                { text: 'Linux price ($ / hour)', flex: 1, xtype: 'templatecolumn', dataIndex: 'priceLinux',
                    tpl: new Ext.XTemplate(
                        '<input class="x-form-text scalr-ui-analytics-pricing-pricefield" style="width: 80px" value="{priceLinux}" data-instanceType="{type}" data-priceType="priceLinux" />'
                    )
                },
                { text: 'Windows price ($ / hour)', flex: 1, xtype: 'templatecolumn', dataIndex: 'priceWindows',
                    tpl: new Ext.XTemplate(
                        '<input class="x-form-text scalr-ui-analytics-pricing-pricefield" style="width: 80px" value="{priceWindows}" data-instanceType="{type}" data-priceType="priceWindows" />'
                    )
                }
            ]
        }]
    ...
    var pricingHistoryPanel = {
        ...
        getHistoryTemplateModel: function (records) {
            Ext.Array.each(records, function (record) {
                record.priceLinux = '<img src="/ui2/images/icons/false.png" />';
                ...
                tpl: [
                    ...
                    '</tpl>',
                    {
                        formatPrice: function (price) {
                            var image = '<img src="/ui2/images/icons/false.png" />';
