Ext.define('Scalr.ui.FarmRoleEditorTab.Mongodb', {
    ...
    showTab: function (record) {
        ...
        if (sslFieldset.rendered) {
            sslFieldset.show();
            sslFieldset[(tabParams.farm.status || 0) > 0 ? 'mask' : 'unmask']();
        } else {
            sslFieldset.setVisible((tabParams.farm.status || 0) == 0);
        }
        this.down('#mongo_ssl_warning').setVisible((tabParams.farm.status || 0) > 0);
    },
        ...
        items: [{
            ...
            items: [{
                ...
                items: [{
                    ...
                    vtype: 'ebssize',
                    getEbsType: function() {
                        return this.up('#mongodb').down('[name="mongodb.data_storage.ebs.type"]').getValue();
                    },
                    getEbsIops: function() {
                        return this.up('#mongodb').down('[name="mongodb.data_storage.ebs.iops"]').getValue();
                    }
                },{
                    xtype: 'label',
                    text: 'GB',
