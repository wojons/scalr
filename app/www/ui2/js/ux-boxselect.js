...
Ext.define('Scalr.ui.tagBox', {
    ...
    initComponent: function () {
        ...
        me.tooltipText = me.tooltipText || 'Add new tag';
        ...
        me.on('change', function (field, value) {
            if (value) {
                ...
                if (me.isTagValid(lastValue)) {
                    ...
                    values.pop();
                    me.setValue(values);
                    Scalr.message.InfoTip('Tag name can only contain letters and numbers,' +
                        ' and must be between 3 and 10 characters long.', me.el);
                }

                me.setBoundListStyle(false);
                me.toolTip.hide();
            }
        });
    ...
    setBoundListStyle: function (isTipVisible) {
        var me = this;
        var boundList = me.getPicker();

        if (boundList) {
            if (isTipVisible) {
                boundList.addCls('scalr-ui-tagbox-boundlist-with-tip');
                boundList.removeCls('scalr-ui-tagbox-boundlist-without-tip');
            } else {
                boundList.addCls('scalr-ui-tagbox-boundlist-without-tip');
                boundList.removeCls('scalr-ui-tagbox-boundlist-with-tip');
            }
        }
    },
    ...
    isTagValid: function (value) {
        ...
        return /^[a-zA-Z0-9-]{3,10}$/.exec(newValue);
    },

    showTooltip: function () {
        var me = this;
        var tooltip = me.toolTip;
        var tagExists = me.isTagExist(me.enteredValue);

        me.setBoundListStyle(me.enteredValue ? !tagExists : false);

        if (me.enteredValue && !tagExists) {
            tooltip.showTooltip(me.enteredValue);
        } else {
            tooltip.hide();
        }
    },
    ...
    afterRender: function () {
        ...
        me.toolTip = Ext.create('Ext.tip.ToolTip', {
            ...
            setTooltipWidth: function () {
                var field = me.el.down('.x-form-item-body');
                var fieldWidth = field.getWidth();

                this.setWidth(fieldWidth);
            },

            setTooltipStyle: function () {
                var boundList = me.getPicker();
                var boundListHeight = boundList.el ? boundList.getHeight() : null;

                if (boundListHeight) {
                    this.addCls('scalr-ui-tagbox-tip-with-list');
                    this.removeCls('scalr-ui-tagbox-tip-without-list');
                } else {
                    this.addCls('scalr-ui-tagbox-tip-without-list');
                    this.removeCls('scalr-ui-tagbox-tip-with-list');
                }
            },

            setTooltipText: function (enteredValue) {
                this.update('<div style="position: absolute; top: 4px; left: 8px;">' +
                    me.tooltipText +
                    ' <span style="color: #FEFDE0; font-weight: bold">\'' +
                    Ext.String.htmlEncode(enteredValue) +
                    '\'</span></div>');
            },
            ...
            getTooltipPosition: function () {
                ...
                if (boundListHeight) {
                    var boundListX = boundList.getX();
                    var boundListY = boundList.getY();

                    return [boundListX, boundListY + boundListHeight];
                }

                var field = me.el.down('.x-form-item-body');
                var fieldX = field.getX();
                var tagBoxY = me.getY();
                var tagBoxHeight = me.getHeight();

                return [fieldX, tagBoxY + tagBoxHeight];
            },

            showTooltip: function (enteredValue) {
                var tooltipPosition = this.getTooltipPosition();

                this.setTooltipWidth();
                this.setTooltipStyle();
                this.setTooltipText(enteredValue);

                this.showAt(tooltipPosition);
            }
        });
